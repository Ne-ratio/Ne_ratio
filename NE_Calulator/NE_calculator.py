...
parser = argparse.ArgumentParser(
    description="Calculate Ne ratio from sliding-windows CSV for Bonobo/Chimpanzee/Human."
)
parser.add_argument(
    "csv",
    help="Path to sliding_windows.csv (contain columns: species, chr_type, pi_value, div_value)"
)
args = parser.parse_args()

if not os.path.isfile(args.csv):
    parser.error(f"no: {args.csv}")
...
block_res.to_csv(r'C:/Users/finni/Desktop/Ne_ratio_with_SE_hominid.csv', float_format='%.4f')
long_for_r = (
    block_res
    .stack(level=0)         
    .reset_index()
